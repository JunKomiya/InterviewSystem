...
def render_interview_view():
    ...
    with col_center:
        with st.container(border=True):
            ...
            st.markdown(
                "<hr style='border: 0.5px solid rgba(0,0,0,0.08)'>", unsafe_allow_html=True)
                ...
                if not user_ans.strip():
                    ...
                    if is_first_turn:
                        ...
                        with st.spinner("AI面接官があなたの回答を評価し、深掘り質問を構成しています..."):
                            conversation_log = [
                                {"speaker": "interviewer",
                                    "text": st.session_state.question_1},
                                {"speaker": "student",
                                    "text": st.session_state.user_answer_1}
                            ]
                        ...
                        filename = os.path.join(
                            TEMP_DIR, f"temp_audio_q2_{timestamp}.mp3")
                        ...
                        if "recorder" in st.session_state and st.session_state.recorder is not None:
                            with st.spinner("カメラをオフにし、結果の解析を行っています..."):
                                ...
                                video_fn = os.path.join(
                                    TEMP_DIR, f"temp_gaze_timelapse_{ts}.webm")
                                map_fn = os.path.join(
                                    TEMP_DIR, f"temp_gaze_map_{ts}.png")

                                # 静止画の生成は高速なので同期で実行
                                st.session_state.recorder.generate_gaze_map(
                                    map_fn)
                                ...
                                if gps:
                                    ...
                                    valid_face_count = sum(
                                        1 for gp in gps if gp.get("is_valid", True))

                                    # 有効フレーム
                                    valid_gaze_frames = [gp for gp in gps if gp.get(
                                        "is_valid", True) and not gp.get("is_blink", False)]

                                    if len(valid_gaze_frames) > 0:
                                        away_count = sum(
                                            1 for gp in valid_gaze_frames if gp.get("looking_away", False))
                                        st.session_state.eye_contact_score = int(
                                            (1 - away_count / len(valid_gaze_frames)) * 100)
                                    else:
                        ...
                        st.session_state.overall_score = eval_json.get(
                            "overall_score", 88)
                        st.session_state.rank = eval_json.get("rank", "A")
                        st.session_state.consistency_score = eval_json.get(
                            "consistency_score", 90)
                        st.session_state.content_quality_score = eval_json.get(
                        ...
                        timestamp = int(time.time())
                        filename = os.path.join(
                            TEMP_DIR, f"temp_audio_eval_{timestamp}.mp3")
                        ...
                        with st.spinner("面接官がフィードバックをまとめています..."):
                            ...
                            use_camera_val = st.session_state.get(
                                "use_camera", True)
